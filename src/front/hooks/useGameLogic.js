...
const useGameLogic = () => {
    ...
    const loadAndShuffleQuestions = async (
        ...
        try {
            ...
            if (finalQuestions.length > 0) {
                ...
                setTimeLeft(15); // Reiniciar tiempo al cargar nuevas preguntas
    ...
    // Cambio para volver a elegir la partida, reiniciciar el juego y limpiar el estado 
    const startNewGame = () => {
        setShuffledQuestions([]); // Limpiar preguntas anteriores
        setQuestionIndex(0); 
        setScore(0); 
        setFeedback(null); 
        setSelectedAnswerButton(null); 
        ...
        setGameStatus("inicio"); 
        setInternalError(null); // Limpiar cualquier error previo
        clearInterval(timerRef.current); // Asegurarse de que el timer se detiene
    };
